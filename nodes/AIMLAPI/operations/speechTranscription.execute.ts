	...
	for (const channel of channels) {
		...
		for (const alternative of alternatives) {
			...
			if (Array.isArray(segments) && segments.length > 0) {
				collected.push(
					...(segments.filter((segment) => typeof segment === 'object') as IDataObject[]),
				);
				continue;
			}
...
async function pollForTranscription(
	...
	for (let attempt = 0; attempt < MAX_POLL_ATTEMPTS; attempt++) {
		...
		const requestOptions = createRequestOptions(
			baseURL,
			`/v1/stt/${encodeURIComponent(generationId)}`,
			'GET',
		);
...
export async function executeSpeechTranscription({
	...
	const inputSource = context.getNodeParameter(
		'transcriptionInputSource',
		itemIndex,
	) as InputSource;
