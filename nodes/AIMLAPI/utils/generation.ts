...
function extractAudioOutputsFromObject(
	objectValue: IDataObject,
	outputs: IDataObject[],
	visited: Set<object>,
) {
...
function extractVideoOutputsFromObject(
	objectValue: IDataObject,
	outputs: IDataObject[],
	visited: Set<object>,
) {
...
function shouldPoll(
	payload: unknown,
	mediaType: MediaType,
): { poll: boolean; generationId?: string } {
...
async function pollForGeneration(
	...
	for (let attempt = 0; attempt < maxAttempts; attempt++) {
		...
		latest = await context.helpers.httpRequestWithAuthentication.call(
			context,
			'aimlApi',
			requestOptions,
		);
